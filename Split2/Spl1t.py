...
class DemoFrame(Frame):
    def __init__(self, screen):
        ...
        layout.add_widget(
            TimePicker("Start Time:", name="start_time_field", on_change=self._on_change, seconds=True), 1)

        layout.add_widget(
            TimePicker("End Time:", name="end_time_field", on_change=self._on_change, seconds=True), 1)

        layout.add_widget(Button("Click Here To Trim", self._split), 1)
    ...
    def _split(self):
        ...
        try:
            ...
            #ffmpeg_command = "ffmpeg -ss " + given_start_time + " -to " + given_end_time + " -i " + given_video_location + " -c copy " + given_output_video_folder
            #print(ffmpeg_command)
            #print(data_dictionary)
            #print(given_video_location)
            #print(given_output_video_folder)
            #print(given_output_video_name)
            #print(given_start_time)
  ...
  #-e 'tell application "Terminal" to do script "echo hello" in front window'
                #quiet shell for ffmpeg, remove the ffmpeg flags in the console.
                #ffmpeg_command #-nostats -loglevel 0
                #ffmpeg -n  -nostats -loglevel 0 -ss 00:00:00 -to 00:00:02 -i input.mkv -c copy output.mkv

                #ffmpeg_command_new = ffmpeg_command[6:]
                #command_header = "ffmpeg -nostats -loglevel 0 "
                #ffmpeg_command_new = command_header + ffmpeg_command_new
                #ffmpeg_command = ffmpeg_command_new
  ...
  -e 'tell application "Terminal" to do script ''' +  "\""+ ffmpeg_command + "\"" + "in front window\'" 
                ...
                #os.system("gnome-terminal -e 'bash -c \""+ffmpeg_command+";bash\"'")
                self.fix()
